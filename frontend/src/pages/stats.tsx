...
export default function Stats() {
    ...
    // Nouvelles données
    const [verbalisations, setVerbalisations] = useState<any[]>([])
    ...
    useEffect(() => {
        fetch('/dpmp-verbalisations.json')
            ...
            .then(data => setVerbalisations(data))
            .catch(error => console.error("Erreur chargement verbalisations:", error))

        fetch('/arrondissements.json')
            .then(response => response.json())
            .then(data => setArrondissements(data))
            .catch(error => console.error("Erreur chargement arrondissements:", error))
    }, [])
    ...
    const networkStats = useMemo(() => {
        ...
        const statsParReseau = stations.reduce((acc, station) => {
            ...
            acc[networkName].total_bikes += (station.free_bikes || 0) + (station.ebikes || 0);
    ...
    const availabilityStats = useMemo(() => {
        ...
        return [
            { name: "Stations avec vélos", value: stations.filter(s => (s.free_bikes || 0) > 0).length },
            { name: "Stations vides", value: stations.filter(s => (s.free_bikes || 0) === 0).length }
        ];
    }, [stations]);
    ...
    const capacityStats = useMemo(() => {
        ...
        })).sort((a, b) => b.capacité - a.capacité).slice(0, 10);
    }, [stations]);

    const bikeTypeDistribution = useMemo(() => {
        if (!stations.length) return [];

        const totalBikes = stations.reduce((sum, s) => sum + (s.free_bikes || 0), 0);
    ...
    return (
        <div className="container mx-auto px-4 py-8 mt-32 max-w-screen-lg">
            <h1 className="text-3xl font-bold mb-8">Statistiques des Vélos en France</h1>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Distribution par Réseau</h2>
                    <ChartContainer config={chartConfig} className="h-[400px] w-full">
                        ...
                        >
                            ...
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="total_bikes" name="Vélos totaux" fill="var(--color-total_bikes)" radius={4} />
                            <Bar dataKey="ebikes" name="Vélos électriques" fill="var(--color-ebikes)" radius={4} />
                        </BarChart>
                    </ChartContainer>
                </div>

                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Infractions par Arrondissement</h2>
                    <ChartContainer config={infractionConfig} className="h-[400px] w-full">
                        <BarChart 
                            width={600} 
                            height={300} 
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Disponibilité des Stations</h2>
                    <ChartContainer config={chartConfig} className="h-[400px] w-full">
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Top 10 Stations par Capacité</h2>
                    <ChartContainer config={chartConfig} className="h-[400px] w-full">
                        <BarChart
                            width={600}
                            height={300}
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Distribution Vélos Classiques vs Électriques</h2>
                    <ChartContainer config={chartConfig} className="h-[400px] w-full">
                        <PieChart width={400} height={300}>
                            <Pie
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">Distribution du Taux de Remplissage</h2>
                    <ChartContainer config={chartConfig} className="h-[400px] w-full">
                        <BarChart
                            width={600}
                            height={300}
